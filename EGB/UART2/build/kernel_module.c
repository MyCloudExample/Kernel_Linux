...
static ssize_t chr_dev_write(struct file *f, const char __user *buff, size_t size, loff_t *off) {
    ...
    // Si ya procesamos los datos, retornar 0
    if (*off > 0) {
        return 0;
    }
    ...
    // Calcular cuánto copiar (máximo buffer size - 1)
    to_copy = min_t(size_t, size, sizeof(shared_buffer) - 1);
    ...
    if (len >= 0 && len < sizeof(shared_buffer)) {
        shared_buffer[len] = '\0';
    }
    
    // Log sin newline
    if (len > 0) {
        char *log_ptr = shared_buffer;
        if (log_ptr[len - 1] == '\n') {
            log_ptr[len - 1] = '\0';
        }
        printk(KERN_INFO "%s: Escrito sobre /dev/%s - %s\n", AUTHOR, CHRDEV_NAME, log_ptr);
    }
    
    // Enviar por UART si está disponible
    if (g_serdev != NULL && len > 0) {
        serdev_device_write_buf(g_serdev, shared_buffer, len);
    }
    
    // Marcar que ya procesamos estos datos
    *off = len;
